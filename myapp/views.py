...
#     unonlion_rooms=Conference.objects.filter(meet_type='线下会议')
...
def xianshangliebiao(request,name):
    xianshangliebiao=Conference.objects.filter(meet_type='线上会议')
    context={
...
def mymeetings(request,pIndex,name):
    mymeetinglist=My_list.objects.all()
    # mymeetlist=[]
    # # my_sub=SUB_NAMES.objects.filter(sub_name=name)
    # # sub_temp=my_sub.meet_name#属于我的会议的名称
    # # for h in sub_temp:
    # #     mine = Conference.objects.get(meet_name=h.meet_name)#遍历当前登录者预约的所有会议
